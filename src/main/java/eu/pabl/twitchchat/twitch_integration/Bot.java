...
public class Bot {
  ...
  public void onJoin(GlobalUserStateEvent event) {
    event.getMessageEvent().getChannelName().ifPresent(channel -> {
      event.getColor().ifPresent(color -> putFormattingColor(event.getDisplayName().orElse(""), color));
      TwitchChatMod.addNotification(Text.translatable("text.twitchchat.bot.connected", channel));
    });
  }
  ...
  public void joinChannel(String channel) {
    ...
    if (this.isConnected()) {
      myExecutor.execute(() -> {
        ...
        String channelID = twitchClient.getHelix().getUsers(this.oauthKey, null, List.of(channel)).execute().getUsers().getFirst().getId();
        twitchClient.getHelix().getGlobalChatBadges(this.oauthKey).execute().getBadgeSets().forEach(Badge::add);
        twitchClient.getHelix().getChannelChatBadges(this.oauthKey, channelID).execute().getBadgeSets().forEach(Badge::add);
      });
    }
  }
}
